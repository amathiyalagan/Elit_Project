...
{
    [TestFixture]
    public class Standard_Commercial_EvaluationTest:ReportsGenerationClass
    {

        Commercial_Evaluation CET;

        [Test, Order(1)]
        [Category("Standard Commercial Evaluation of Suppliers")]
        public void Commercial_Evaluation_Supplier()
        {
            ...
            try
            {
                ...
                CET.Quotation();
                CET.Closed_Document();
                CET.Closed_Document_Search("STANDARD RFQ CREATION 35 of 40 on JAN2022");  //line no 50
                CET.Document();
                CET.Doc_Action();
                CET.UnLock_Commercial();
                _test.Log(Status.Info, "Commercial Evaluation Unlocked");
                CET.Buyer_LogOut();
                ...
                CET.Password("Atsdxb@003");
                CET.SignIn();
                _test.Log(Status.Info, "Commercial Approver Logged in");
                CET.Sourcing();
                CET.Quotation();
                CET.Closed_Document();
                CET.Closed_Document_Search("STANDARD RFQ CREATION 35 of 40 on JAN2022");
                CET.Document();
                CET.Doc_Action();
                ...
                _test.Log(Status.Info, "Commercial rating is Submitted");
                CET.Comm_Eval_Logout();
                _test.Log(Status.Info, "commercial Approver Logged out");
            }
            catch (Exception ex)
            {
